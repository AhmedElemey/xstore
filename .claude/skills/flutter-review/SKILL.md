...
### 2026-07-25 — Never add a "debug convenience" that unredacts a secret in the logger
- **What happened:** `LoggingInterceptor._redact` had a `skipRedact = kDebugMode && key == 'x-auth-token'` branch that printed the per-user session JWT in cleartext in every debug build "for backend verification" — a standing secret leak into device logs, and it broke the redaction contract test.
- **Rule:** The request/response logger redacts every entry in `_redactedHeaderKeys`/`_redactedBodyKeys` unconditionally — no per-key `kDebugMode` escape hatch. To inspect a token during a one-off debug session, print it locally at the call site and delete it; don't weaken the central redactor. `X-Auth-Token` carries the session JWT and stays redacted like `Authorization`.
- **Where it applies:** `logging_interceptor.dart`, any future redaction allow/deny logic.

### 2026-07-25 — Edit-profile fields that the update-profile contract can't carry must not look editable
- **What happened:** The Edit Profile screen exposes `email`, `phone`, `bio`, and `store category` as editable inputs, and `_profileEditEqualsUser` compares all four — so editing any of them enables Save, but `toUpdateProfileRequest()` omits email/phone/bio entirely and sends the *existing* `storeCategoryId` (the picker only sets a hardcoded label string, never an id). Result: the edit appears to save, then reverts on the post-save refresh.
- **Rule:** Every editable control on the profile edit form must map to a field `updateProfileWireFields` actually sends. If the backend `UpdateProfileRequest` can't carry it (email/phone/bio) render it read-only/display-only; if it needs an id (store category), the picker must set the id (`storeCategoryId`), not a label string, and `toUpdateProfileRequest` must forward the edited id. Don't let a field flip `hasChanges` unless save persists it.
- **Where it applies:** `edit_profile_screen.dart`, `profile_state.dart` (`toUpdateProfileRequest`, `_profileEditEqualsUser`), `profile_remote_datasource.dart` (`updateProfileWireFields`).
