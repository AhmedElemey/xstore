...
### 2026-07-28 — Changing a role's bottom-nav tabs must keep shell + nav + role-home in sync
- **What happened:** Removed Home/Explore from the vendor bottom nav and added a Delivery Requests tab. The nav is index-mapped 1:1 to the role's shell branches, and the redirect's `roleHome` was hardcoded `user.isCourier ? deliveries : home` — so dropping `/home` from the vendor shell would have sent vendors to a route that no longer exists in their tree ("no route" error) on login and on every consumer/courier-restricted bounce.
- **Rule:** To change a role's tabs, edit in lockstep: (1) that role's `_xShellBranches()` list in `app_router.dart` (the tab's screen must be a shell branch — a top-level route can't also be a branch at the same path, so give the new tab a distinct path, e.g. `vendorDeliveryRequests`); (2) the matching `labels`/`icons` switch arm in `xstore_bottom_nav.dart` (same order/length) and any hardcoded accent index; (3) `computeXStoreAuthRedirect`'s `roleHome` — make it a `switch (user.role)` returning a route that EXISTS in that role's shell, and add a `role != consumer && (loc==home||loc==explore) → roleHome` bounce so stale `context.go(home)` in auth screens is corrected; (4) `isXRestrictedRoute` for the new route so non-members deep-linking get redirected, not a no-route. Grep tests for the old redirect target — a test asserting the old `roleHome` (e.g. vendor→/home) must be updated to the new one (vendor→/vendor-orders). For a screen reused as both a pushed route and a shell tab, return via `context.pop()` (guarded by `canPop`) instead of `pushReplacement(fixedRoute)` so it stays inside whichever shell/stack opened it.
- **Where it applies:** `app_router.dart`, `xstore_bottom_nav.dart`, `router_notifier.dart`, `app_routes.dart`, `send_package_screen.dart`; any future role tab change.

### 2026-07-27 — Opening a role-gated route to a second role touches four coupled places
- **What happened:** "Send a package" (send-package / my-packages) was consumer-only; opening it to vendors required more than the UI button. The route stayed blocked until every gate agreed.
- **Rule:** To let another role into an existing role-gated feature, change all four in lockstep: (1) the feature provider's role gate (`delivery_requests_provider._consumer` — return the user for consumer OR vendor); (2) the route-classification helper in `app_routes.dart` (pull the routes out of `isConsumerRestrictedRoute` into a `isRequesterRestrictedRoute` that both roles pass); (3) the redirect enforcement in `router_notifier.computeXStoreAuthRedirect` (add the new gate, still bouncing the roles that must NOT enter — courier/admin); (4) the entry-point menu tile for the newly-allowed role (`profile_menu_blocks.dart` vendor branch). Reuse an existing l10n key (`menuMyPackages`) rather than adding arb keys for a same-flow reuse. Add a `computeXStoreAuthRedirect` test asserting the newly-allowed role gets `isNull` and the still-blocked role gets `isNotNull`.
- **Where it applies:** `delivery_requests_provider.dart`, `app_routes.dart`, `router_notifier.dart`, `profile_menu_blocks.dart`; any future "open route X to role Y".
