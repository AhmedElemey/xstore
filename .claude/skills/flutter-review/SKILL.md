...
- **Partially fixed 2026-08-03:** `pauseListing` now goes through the dedicated, confirmed `PUT /api/listings/{id}/deactivate` (`ApiEndpoints.apiListingDeactivate`, `ListingRemoteDataSource.deactivateListing`) — a bodyless status-only PUT that never touches `imageFiles`, so pause no longer carries this risk. `resumeListing` still has no confirmed dedicated "activate" endpoint in the collection and was deliberately NOT given a guessed one; it still runs through the generic multipart update with `imagePaths: const []`, so the risk remains for resume specifically until the backend is live-probed or a real activate endpoint is confirmed. `_mutateListingStatus` was refactored into `_applyOptimisticStatusMutation` (shared optimistic-update/rollback scaffolding, network call injected per caller) so pause/resume no longer share a single network call.
...
- **Left open, deliberately:** (4) `deleteListing` was NOT changed — swapping it for `PUT .../cancel` would be a real behavior change (soft-cancel vs hard-delete) that's unverifiable without a live backend (hosted backend was 500.30 "app failed to start" the whole session, confirmed via a fresh probe on 2026-08-03), and guessing wrong on a destructive action is worse than leaving a flagged assumption. Needs a live probe (or backend team confirmation) before touching.
