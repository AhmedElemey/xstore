...
- **Rule:** Check for a working `dart`/`flutter` binary before planning a codegen-touching change. If absent, hand-edit the generated `.freezed.dart`/`.g.dart` file with a scoped, mechanical rename (e.g. `sed` over just the renamed identifiers, verified with a follow-up grep for leftovers) rather than skipping the regen — freezed output is a deterministic mirror of the field list, so a careful identifier-for-identifier substitution is safe when the rename doesn't add/remove/retype fields. State plainly in the final report that `flutter analyze`/`flutter test` could not be run and verification was manual (read-through + grep), never claim a green build you didn't observe.
- **Where it applies:** Any field rename or freezed/riverpod-codegen touching change; report language whenever the toolchain is unavailable.
