...
- **Fixed 2026-08-03 (logging + register-failure test, item 4):** `logging_interceptor.dart` no longer has the `skipRedact` debug-mode carve-out that let `X-Auth-Token` through unredacted — see the new dedicated lesson below. `auth_redirect_and_providers_test.dart`'s register-failure test now calls `rn.updateStoreLocation(storeCityId: 1, storeGovernmentId: 1)` before the step-2 `nextStep` check, matching the step-2 `storeLocation` validation requirement added 2026-07-26 (the test had never been updated for it).
...
### 2026-08-03 — Never carve out a "debug-only" exception to secret redaction
- **What happened:** `logging_interceptor.dart`'s `_redact` had `final skipRedact = kDebugMode && keyLower == 'x-auth-token';`, added "for backend verification" so the live session token would print in full during debugging — but `flutter test` also runs with `kDebugMode == true`, so it silently broke the interceptor's own test (`onRequest redacts Authorization header and sensitive body fields`) the day it was added, and stayed broken for over a week unnoticed. A second, separate leak of the same header existed in `dio_provider.dart`'s request interceptor (`debugPrint('X-Auth-Token: ${options.headers['X-Auth-Token']}')`), bypassing the logging interceptor's redaction entirely.
- **Rule:** Never gate secret redaction on `kDebugMode` (or any build-mode flag) — debug logs end up in `adb logcat`, crash reporters, and screen recordings just as easily as release ones, and `kDebugMode` is also `true` under `flutter test`, so any such carve-out silently breaks whatever test asserts the redaction. If a value must be visible for debugging, log a masked/truncated form (e.g. last 4 chars) or log "header present: yes/no" instead of the raw secret — never the literal value. When redaction lives in a shared interceptor, that's the ONLY place a secret should ever be printed — grep for duplicate raw `debugPrint`s of the same header/field elsewhere in the request pipeline.
- **Where it applies:** `logging_interceptor.dart`, `dio_provider.dart`, any future interceptor or debug print touching `X-Auth-Token`, `Authorization`, or body fields in `_redactedBodyKeys`.
