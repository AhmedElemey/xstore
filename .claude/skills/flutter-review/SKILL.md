...
### 2026-07-27 — Swapping a mock-only feature to a real backend: interface + MockConfig branch
- **What happened:** The delivery feature had only `DeliveryRequestMockDataSource` (returning entities directly, no model layer) wired as a keepAlive "server". Adding the real backend meant introducing a live datasource without disturbing the domain/providers/UI or the widget tests (which override at the *repository* level).
- **Rule:** To back a mock-only feature with a live API: (1) extract a shared `abstract interface class XDataSource` (put it in the new `x_remote_datasource.dart`; the mock `implements` it — add `@override` for `annotate_overrides`); (2) new `XRemoteDataSource` returns domain entities directly if the feature has no model layer (match the feature's existing pattern — don't add models), parsing wire enums by `.name` with a safe fallback and dates via `DateTime.tryParse(...)?.toLocal()`; (3) repository field becomes the interface type and `_guard` maps `NetworkException`/`UnauthorizedException`/`AppException` (in that order — specific before general) to typed `Failure`s; (4) dependencies provider branches `MockConfig.useMock ? Mock() : Remote(ref.watch(dioProvider))` — real is the default. Token-scoped backends ignore id args on the wire (use `/mine`, `/courier/mine`); document that the `consumerId`/`courierId` params are unused.
- **Where it applies:** `delivery_request_remote_datasource.dart`, `delivery_request_datasource.dart`, `delivery_request_repository_impl.dart`, `delivery_request_dependencies.dart`; any future mock→live datasource swap.

### 2026-07-27 — mapDioException reads ASP.NET ProblemDetails title/detail
- **What happened:** The .NET delivery backend returns guard failures (404/409/403) as ProblemDetails `{status,title,type}`, but `_serverErrorMessage` only read `error`/`message`, so a 409 "Invalid transition…" surfaced as generic Dio boilerplate.
- **Rule:** `_serverErrorMessage` now also reads ProblemDetails `detail` then `title`, but skips the generic `"One or more validation errors occurred."` title so the 400/422 branch's field-level `errors` still win. Any new ASP.NET endpoint's typed guard message will now surface as-is.
- **Where it applies:** `dio_error_mapper.dart`, `test/dio_error_mapper_test.dart`.

### 2026-07-27 — A new MockConfig branch in a provider breaks repo-stub tests in live (default) mode
- **What happened:** Wiring the courier cash wallet, `courierCashWalletProvider` gained an `if (MockConfig.useMock) {…getCourierOrders…} else {…getCourierCashInHandOrders…}` branch. `flutter test` (no define) runs LIVE, so the wallet + courier-deliveries screen tests — which override `ordersRepositoryProvider` with `StubOrdersRepository` but only supplied `getCourierOrdersResult` — took the live branch, hit the stub's default `Left`, and the provider threw. The tests passed in `--dart-define=MOCK=true` but failed in the default CI job.
- **Rule:** When a provider starts branching on `MockConfig.useMock` to call a DIFFERENT repository method per mode, every test that stubs that repository must supply results for BOTH paths (add the new field to the stub, e.g. `getCashInHandResult`, and set it in each override). Default `flutter test` = live mode, so the live path is the one that must be green by default. Run new/changed provider tests in BOTH modes before declaring done.
- **Where it applies:** `courier_cash_wallet_provider.dart`, `stub_orders_repository.dart`, `courier_cash_wallet_provider_test.dart`, `courier_deliveries_screen_test.dart`; any provider that gains a mock/live datasource split.

### 2026-07-27 — Match a new backend Order DTO to the app's existing `_orderFromApiMap`
- **What happened:** The new .NET courier endpoints could have returned any JSON, but the app already parses orders via `_orderFromApiMap` / `OrderItemModel` (items keyed `listingName`/`listingImage`/`price`/`total`, status/paymentMethod lowercased). Inventing a different shape would have silently blanked item names/prices on the courier cards.
- **Rule:** When adding a backend that a mature client already parses, shape the response to the client's EXISTING parser rather than editing the parser — verify the exact keys (`listingName` not `title`, `price`/`total` not `unitPrice`) and that enum casing survives the client's `.toLowerCase()` switch. Repoint only the courier-session order routes; the shared vendor `markShipped`/`markDelivered` use cases must be left alone (add courier-specific repo methods instead).
- **Where it applies:** `orders_remote_datasource.dart` (courier methods), `orders_repository.dart`, the XStoreDelivery backend `OrderDto`.
