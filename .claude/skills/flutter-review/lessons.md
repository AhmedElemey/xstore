...
- **Rule:** `_handleGoogleSuccess` calls `checkGoogleUser` (matches Google-linked identities, not emails). Only check-user decides — Firebase `isNewUser` is ignored (user decision). A hit with a role logs in with it; `exists: true` with an unreadable role logs in as consumer, retrying vendor on a "different role" error. A miss (or failed lookup) sets `pendingSocialResult` + `needsRoleSelection`; the router shows `SocialRoleScreen`, and `completeSocialRegistration` calls only the chosen role's endpoint (never retries the other role — that would create the wrong account), clearing `needsRoleSelection` before `adoptSession` so the redirect goes home. Parse `exists`/`role`/`roleName` tolerantly and unwrap `{isSuccess, data: {token}}`. Notifiers never navigate via `goRouterProvider` (cycle through `routerNotifierProvider`); set state and let the redirect or a widget `ref.listen` act. Hand-written `implements AuthRepository`/`AuthRemoteDataSource` doubles that don't extend `Fake` need every new interface method.
