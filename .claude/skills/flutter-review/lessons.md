...
- **Rule:** Login posts `phoneNumber` (normalized 11-digit local, via `AppValidators.normalizeEgyptLocal`) — `LoginParams.emailOrPhone` is a historical name. Consumer register requires `email`; vendor register is multipart with a required `profileImage` file; `dateOfBirth` is date-only `YYYY-MM-DD`. Passwordless login is `send-login-otp` + `login-with-otp`. Password reset (`forgot-password`) is email-only. Google: `POST /api/auth/google/check-user` (`{idToken, clientId}` → `{exists, role}`, read-only) runs first; if the account exists, log in via the role-specific `google/{consumer|vendor}/login` with the returned role; a new identity picks a type on the social role screen, and that role's endpoint creates the account and logs in (no phone or password is collected; the profile's add-phone prompt covers it). Trust get-profile for the role. Reconcile against the Postman collection by probing each endpoint.
...
### 2026-09-12 — Google sign-in: existing users log in, new users pick a type
- **Rule:** `_handleGoogleSuccess` calls `checkGoogleUser` (matches Google-linked identities, not emails). A hit logs in with the returned role. On a miss, Firebase `isNewUser: false` means a returning identity (e.g. an email account sharing the Gmail): log in as consumer, retrying vendor on a "different role" error (and the reverse). `isNewUser: true` plus a miss sets `pendingSocialResult` + `needsRoleSelection`; the router shows `SocialRoleScreen`, and `completeSocialRegistration` calls only the chosen role's endpoint (never retries the other role — that would create the wrong account), clearing `needsRoleSelection` before `adoptSession` so the redirect goes home. Parse `exists`/`role`/`roleName` tolerantly and unwrap `{isSuccess, data: {token}}`. Notifiers never navigate via `goRouterProvider` (cycle through `routerNotifierProvider`); set state and let the redirect or a widget `ref.listen` act. Hand-written `implements AuthRepository`/`AuthRemoteDataSource` doubles that don't extend `Fake` need every new interface method.
- **Where it applies:** `social_auth_provider.dart`, `social_role_screen.dart`, `router_notifier.dart`, `login_screen.dart`, social/login tests.
