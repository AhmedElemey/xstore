...
- **Rule:** The Orbit redesign is dark-first (`AppThemeMode` defaults to dark; a saved choice wins). Use theme colours (`context.primaryColor` = plasma in dark, violet in light) — `AppColors.plasma` can't carry white text, so never use it as a light-mode fill. Money/COD amounts use `AppColors.cash`. Screen backdrops wrap the Scaffold `body` in `SpaceBackground`; keep Scaffolds opaque, because a transparent Scaffold lets the previous route show through during push transitions and un-pins sticky headers that paint `context.backgroundColor`. Replace hardcoded `AppColors.light*` in widgets with `context.*` getters so dark mode holds.
