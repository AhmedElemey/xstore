...
- **Rule:** Many files here aren't at the default 80-column width, so `dart format` on a whole file (or a batch) produces hundreds of unrelated changed lines. Check `git diff --stat` after formatting; if it's out of proportion, restore the file (`git show HEAD:<path>`, or a pre-format copy when it has other uncommitted edits) and reapply the edit by hand. A slightly misindented wrapped block is better than a 100-line unrelated diff.
...
- **Rule:** `PUT /api/vendor/orders/status` body is `{orderIds: [...], status: "Confirmed"}` using `orderStatusToWireName` (Pending/Confirmed/Processing/Shipped/Delivered/Cancelled) at the JSON root — not the int code (that's `orderStatusToWire`, for GET parsing) and not wrapped in `{request: ...}`.
...
### 2026-09-10 — Localization gaps: legacy constants and constructor defaults
- **Rule:** Replace `AppStrings.*` with `context.l10n.*` wherever a key exists. A widget parameter whose default must be localized becomes nullable and resolves `param ?? context.l10n.key` in `build()` (const defaults can't use context) — e.g. `ErrorStateWidget.retryLabel`. Grep `= AppStrings\.` and string-literal defaults in `lib/shared/widgets/**`. l10n audits must include dialogs, sheets and interpolated labels in secondary screens. Use `MediaQuery.sizeOf`, not `MediaQuery.of(context).size`.
- **Where it applies:** Shared widgets and all screens.
