// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'cart_dependencies.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$cartRemoteDataSourceHash() =>
    r'fd6775b5c334df67abd2f24460acd6b75ce4b78f';

/// See also [cartRemoteDataSource].
@ProviderFor(cartRemoteDataSource)
final cartRemoteDataSourceProvider = Provider<CartRemoteDataSource>.internal(
  cartRemoteDataSource,
  name: r'cartRemoteDataSourceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$cartRemoteDataSourceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef CartRemoteDataSourceRef = ProviderRef<CartRemoteDataSource>;
String _$cartRepositoryHash() => r'1ffb89a993eb3b2ac186191ecc2825ab028dc625';

/// See also [cartRepository].
@ProviderFor(cartRepository)
final cartRepositoryProvider = Provider<CartRepository>.internal(
  cartRepository,
  name: r'cartRepositoryProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$cartRepositoryHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef CartRepositoryRef = ProviderRef<CartRepository>;
String _$getCartUseCaseHash() => r'3fc7450ce31d5780d7d156e3aea7fff7f32dc262';

/// See also [getCartUseCase].
@ProviderFor(getCartUseCase)
final getCartUseCaseProvider = AutoDisposeProvider<GetCartUseCase>.internal(
  getCartUseCase,
  name: r'getCartUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$getCartUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef GetCartUseCaseRef = AutoDisposeProviderRef<GetCartUseCase>;
String _$addToCartUseCaseHash() => r'dda555d8316281d745d6d80b03d7842f16528a59';

/// See also [addToCartUseCase].
@ProviderFor(addToCartUseCase)
final addToCartUseCaseProvider = AutoDisposeProvider<AddToCartUseCase>.internal(
  addToCartUseCase,
  name: r'addToCartUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$addToCartUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef AddToCartUseCaseRef = AutoDisposeProviderRef<AddToCartUseCase>;
String _$addOrUpdateCartItemUseCaseHash() =>
    r'eedee6b794af8cef498b5f88c013b6cfa02b334e';

/// See also [addOrUpdateCartItemUseCase].
@ProviderFor(addOrUpdateCartItemUseCase)
final addOrUpdateCartItemUseCaseProvider =
    AutoDisposeProvider<AddOrUpdateCartItemUseCase>.internal(
  addOrUpdateCartItemUseCase,
  name: r'addOrUpdateCartItemUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$addOrUpdateCartItemUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef AddOrUpdateCartItemUseCaseRef
    = AutoDisposeProviderRef<AddOrUpdateCartItemUseCase>;
String _$removeFromCartUseCaseHash() =>
    r'eb281e147153f18bf5b470750d1a3729001a8160';

/// See also [removeFromCartUseCase].
@ProviderFor(removeFromCartUseCase)
final removeFromCartUseCaseProvider =
    AutoDisposeProvider<RemoveFromCartUseCase>.internal(
  removeFromCartUseCase,
  name: r'removeFromCartUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$removeFromCartUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef RemoveFromCartUseCaseRef
    = AutoDisposeProviderRef<RemoveFromCartUseCase>;
String _$updateQuantityUseCaseHash() =>
    r'5d41c9d94fbc1cf72efa322917929dd5138e0bf7';

/// See also [updateQuantityUseCase].
@ProviderFor(updateQuantityUseCase)
final updateQuantityUseCaseProvider =
    AutoDisposeProvider<UpdateQuantityUseCase>.internal(
  updateQuantityUseCase,
  name: r'updateQuantityUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$updateQuantityUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef UpdateQuantityUseCaseRef
    = AutoDisposeProviderRef<UpdateQuantityUseCase>;
String _$clearCartUseCaseHash() => r'2db472780e7cd9375212b8c8cdf55b4a1179ba1e';

/// See also [clearCartUseCase].
@ProviderFor(clearCartUseCase)
final clearCartUseCaseProvider = AutoDisposeProvider<ClearCartUseCase>.internal(
  clearCartUseCase,
  name: r'clearCartUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$clearCartUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ClearCartUseCaseRef = AutoDisposeProviderRef<ClearCartUseCase>;
String _$applyCouponUseCaseHash() =>
    r'b88a7298d4922e8b8b2315c8fd6795d997ad5414';

/// See also [applyCouponUseCase].
@ProviderFor(applyCouponUseCase)
final applyCouponUseCaseProvider =
    AutoDisposeProvider<ApplyCouponUseCase>.internal(
  applyCouponUseCase,
  name: r'applyCouponUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$applyCouponUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef ApplyCouponUseCaseRef = AutoDisposeProviderRef<ApplyCouponUseCase>;
String _$removeCouponUseCaseHash() =>
    r'614f93ce4b1e8cc753371094a7a1e0967359c745';

/// See also [removeCouponUseCase].
@ProviderFor(removeCouponUseCase)
final removeCouponUseCaseProvider =
    AutoDisposeProvider<RemoveCouponUseCase>.internal(
  removeCouponUseCase,
  name: r'removeCouponUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$removeCouponUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef RemoveCouponUseCaseRef = AutoDisposeProviderRef<RemoveCouponUseCase>;
String _$placeOrderUseCaseHash() => r'18f472dbc8dc38bfbc59c2c9387848b5a28947bc';

/// See also [placeOrderUseCase].
@ProviderFor(placeOrderUseCase)
final placeOrderUseCaseProvider =
    AutoDisposeProvider<PlaceOrderUseCase>.internal(
  placeOrderUseCase,
  name: r'placeOrderUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$placeOrderUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef PlaceOrderUseCaseRef = AutoDisposeProviderRef<PlaceOrderUseCase>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member
