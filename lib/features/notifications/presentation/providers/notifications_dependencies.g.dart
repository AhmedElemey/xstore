// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'notifications_dependencies.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$notificationsRemoteDataSourceHash() =>
    r'6409ea06dbcfbfb257bcecb2c678ebb6247a6b5e';

/// See also [notificationsRemoteDataSource].
@ProviderFor(notificationsRemoteDataSource)
final notificationsRemoteDataSourceProvider =
    Provider<NotificationsRemoteDataSource>.internal(
  notificationsRemoteDataSource,
  name: r'notificationsRemoteDataSourceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$notificationsRemoteDataSourceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef NotificationsRemoteDataSourceRef
    = ProviderRef<NotificationsRemoteDataSource>;
String _$notificationsRepositoryHash() =>
    r'da3d1ab55a192603189bd4f9cb34825d6a01cf9f';

/// See also [notificationsRepository].
@ProviderFor(notificationsRepository)
final notificationsRepositoryProvider =
    Provider<NotificationsRepository>.internal(
  notificationsRepository,
  name: r'notificationsRepositoryProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$notificationsRepositoryHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef NotificationsRepositoryRef = ProviderRef<NotificationsRepository>;
String _$getNotificationsUseCaseHash() =>
    r'0e9bafcec0774e00ecab143573381ab50124c96e';

/// See also [getNotificationsUseCase].
@ProviderFor(getNotificationsUseCase)
final getNotificationsUseCaseProvider =
    AutoDisposeProvider<GetNotificationsUseCase>.internal(
  getNotificationsUseCase,
  name: r'getNotificationsUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$getNotificationsUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef GetNotificationsUseCaseRef
    = AutoDisposeProviderRef<GetNotificationsUseCase>;
String _$markReadUseCaseHash() => r'c7f2f86377140a70b9fd8ca54de253811d31615b';

/// See also [markReadUseCase].
@ProviderFor(markReadUseCase)
final markReadUseCaseProvider = AutoDisposeProvider<MarkReadUseCase>.internal(
  markReadUseCase,
  name: r'markReadUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$markReadUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef MarkReadUseCaseRef = AutoDisposeProviderRef<MarkReadUseCase>;
String _$markAllReadUseCaseHash() =>
    r'2b44b4812e87a5cf562b15f322e649a20f2ed106';

/// See also [markAllReadUseCase].
@ProviderFor(markAllReadUseCase)
final markAllReadUseCaseProvider =
    AutoDisposeProvider<MarkAllReadUseCase>.internal(
  markAllReadUseCase,
  name: r'markAllReadUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$markAllReadUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef MarkAllReadUseCaseRef = AutoDisposeProviderRef<MarkAllReadUseCase>;
String _$markUnreadUseCaseHash() => r'79371133dbe5867011bdde0ac49cbfb860cc4b11';

/// See also [markUnreadUseCase].
@ProviderFor(markUnreadUseCase)
final markUnreadUseCaseProvider =
    AutoDisposeProvider<MarkUnreadUseCase>.internal(
  markUnreadUseCase,
  name: r'markUnreadUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$markUnreadUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef MarkUnreadUseCaseRef = AutoDisposeProviderRef<MarkUnreadUseCase>;
String _$deleteNotificationUseCaseHash() =>
    r'9a9e97859d31f144d9feb4209628ca4c1c9122c6';

/// See also [deleteNotificationUseCase].
@ProviderFor(deleteNotificationUseCase)
final deleteNotificationUseCaseProvider =
    AutoDisposeProvider<DeleteNotificationUseCase>.internal(
  deleteNotificationUseCase,
  name: r'deleteNotificationUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$deleteNotificationUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

typedef DeleteNotificationUseCaseRef
    = AutoDisposeProviderRef<DeleteNotificationUseCase>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member
