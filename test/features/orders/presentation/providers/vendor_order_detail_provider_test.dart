...
// then refetches its own order. It must NOT invalidate vendorOrdersProvider:
// that keepAlive StateNotifier resets to empty and the Incoming Orders tab
// stays mounted, so the list would stay blank until a remount.
  ...
  group('VendorOrderDetailNotifier mutations delegate to VendorOrdersNotifier', () {
    test(
      ...
      () async {
        ...
        // VendorOrdersNotifier already _mergeOrder's the confirmed row. Do not
        // invalidate the keepAlive list — that would wipe Incoming Orders while
        // the shell tab stays mounted.
        expect(
          container.read(vendorOrdersProvider).orders.single.status,
          OrderStatus.confirmed,
          reason: 'list stays populated after confirm; invalidate is not used',
        );
      },
    );

    test(
      'rejectOrder failure leaves both the list and the detail order untouched',
      () async {
        final container = await _syncedContainer(
      ...
      () async {
        final container = await _syncedContainer(
          markProcessing: (orderId) =>
              Right(_order(status: OrderStatus.processing)),
